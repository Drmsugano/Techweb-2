...
class VendedorDao extends Dao
{
    ...
    public function read_all()
    {
        try {
            $pdo = $this->conexao->get_pdo();
            $pdo_sql = $pdo->prepare("SELECT vd.id, vd.nome, vd.nivel, e.nome AS equipe FROM vendedor vd INNER JOIN equipe e ON e.id = vd.equipe_id;");
            $pdo_sql->execute();
    ...
    public function read($id)
    {
        try {
            ...
            foreach ($array_vnd as $lista) {
                ...
                $vendedor->nome = $lista['nome'];
                $vendedor->nivel = $lista['nivel'];
                $vendedor->equipe = $lista['equipe_id'];
                $vendedores[] = $vendedor;
            }
            return $vendedores;
        } catch (PDOException $ex) {
